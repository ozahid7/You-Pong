...
const OverviewCard = (props: { loses: number; wins: number }) => {
	let result: number;
	result =
		props.wins + props.loses != 0
			? Math.round((props.wins * 100) / (props.wins + props.loses))
			: 50;

	const winval = props.loses === 0 && props.wins === 0 ? 1 : props.wins;
	const loseval = props.loses === 0 && props.wins === 0 ? 1 : props.loses;
