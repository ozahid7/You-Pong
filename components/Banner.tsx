...
const Banner = ({ isGreen }: { isGreen: boolean }) => {
    ...
    const customClass = `w-[6%] sm:w-[3%] max-w-[600px] max-h-[100px] sm:min-h-full bg-palette-${isGreen ? "green" : "orange"}`;

    return (
        <div className=" flex overflow-clip rounded-sm h-[12%] w-[60%] min-w-[260px]">
            <div className={customClass}></div>
            <div
                className={`w-full max-w-[600px] max-h-[100px] sm:min-h-full  bg-gradient-to-r from-[${start}] to-[${end}] pl-6 flex items-center justify-between pr-6`}
            >
