...
function Map({ elements, border, background, name, handelClick }: Props) {
    ...
    return (
        <div
            onClick={handelClick}
            className="flex h-1 min-h-[110px] sm:min-h-[160px] lg:min-h-[200px] h:w-[50%]  w-[70%] md:w-[30%] md max-w-[300px] justify-center flex-col items-center"
        >
            <div
                style={{ background: `${background}` }}
                className={`flex w-[85%] h-full shadow-md z-10 justify-center items-center  flex-col rounded-sm`}
            >
                <button
                    style={{ borderColor: `${border}` }}
                    className={`flex shadow-lg rounded-sm focus:ring-[6px] sm:focus:ring-[10px] lg:focus:ring-[12px] focus:ring-palette-orange w-[90%] h-[90%] outline-none  lg:border-4 items-center justify-center hover:scale-110 transform-gpu btn btn-ghost`}
                >
